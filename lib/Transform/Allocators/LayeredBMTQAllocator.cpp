...
// --------------------- LayeredBMTQAllocator ------------------------
LayeredBMTQAllocator::LayeredBMTQAllocator(ArchGraph::sRef ag)
    : QbitAllocator(ag),
      mGen((std::random_device())()),
      mDistribution(0.0) {}

std::vector<MappingCandidate>
LayeredBMTQAllocator::extendCandidates(const Dep& dep,
...
void LayeredBMTQAllocator::resetCandidatesWeight(std::vector<MappingCandidate>& candidates) {
    for (auto& candidate : candidates) {
        candidate.weight = 0;
    }
}

void LayeredBMTQAllocator::setCandidatesWeight(std::vector<MappingCandidate>& candidates,
                                           Graph& lastPartitionGraph) {
    resetCandidatesWeight(candidates);
...
std::vector<MappingCandidate>
LayeredBMTQAllocator::filterCandidates(const std::vector<MappingCandidate>& candidates) {
...
std::vector<std::vector<MappingCandidate>> LayeredBMTQAllocator::phase1(QModule::Ref qmod) {
    // First Phase:
    ...
    for (auto& l : mLayers) {
        ...
        bool hasAtLeastOneDep = false;

        for (auto& node : l) {
            auto dependencies = mDBuilder.getDeps(node);

            if (dependencies.empty()) continue;

            hasAtLeastOneDep = true;
            auto dep = dependencies[0];

            candidates = extendCandidates(dep, mapped, candidates);
            EfdAbortIf(candidates.empty(), "Could not extend the partition.");

            setCandidatesWeight(candidates, lastPartitionGraph);
...
uint32_t LayeredBMTQAllocator::getNearest(uint32_t u, const InverseMap& inv) {
    uint32_t minV = 0;
...
void LayeredBMTQAllocator::propagateLiveQubits(const Mapping& fromM, Mapping& toM) {
...
uint32_t LayeredBMTQAllocator::estimateSwapCost(const Mapping& fromM, const Mapping& toM) {
...
std::vector<Mapping>
LayeredBMTQAllocator::tracebackPath(const std::vector<std::vector<TracebackInfo>>& mem,
                                uint32_t idx) {
    std::vector<Mapping> mappings;
...
SwapSeq LayeredBMTQAllocator::getTransformingSwapsFor(const Mapping& fromM,
                                                  Mapping toM) {
...
void LayeredBMTQAllocator::normalize(MappingSwapSequence& mss) {
...
MappingSwapSequence
LayeredBMTQAllocator::phase2(const std::vector<std::vector<MappingCandidate>>& collection) {
    // Second Phase:
...
Mapping LayeredBMTQAllocator::phase3(QModule::Ref qmod, const MappingSwapSequence& mss) {
    // Third Phase:
    ...
    INF << "Layers: " << mLayers.size() << std::endl;
    INF << "Mappings: " << mss.mappings.size() << std::endl;
...
void LayeredBMTQAllocator::init(QModule::Ref qmod) {
    ...
    mLayers = PassCache::Get<LayersBuilderPass>(qmod)->getData();
...
Mapping LayeredBMTQAllocator::allocate(QModule::Ref qmod) {
    init(qmod);
...
LayeredBMTQAllocator::uRef LayeredBMTQAllocator::Create(ArchGraph::sRef ag) {
    return uRef(new LayeredBMTQAllocator(ag));
}
